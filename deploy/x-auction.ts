...
const beforeCallback = async () => {
  const SettlementVault =
    await hh.companionNetworks.holesky.deployments.getOrNull("SettlementVault");
  if (!SettlementVault) {
...
const afterCallback = async (deployments: any, deployResult: any) => {
  ...
  if (!isInitiated) {
    const receipt = await AuctionContract.confidentialConstructor.sendCCR().then((tx) => tx.wait());
    if (receipt.status == 0) throw new Error("ConfidentialInit callback failed");
  }

  deployments.log("\t2.) Registering auction-master for SettlementVault");
  const auctionMaster = await AuctionContract.auctionMaster();
  const settlementVaultDeployment =
    await hh.companionNetworks.holesky.deployments.get("SettlementVault");
  ...
  const registerReceipt = await SettlementVault.registerAuctionMaster(auctionMaster).then((tx) =>
    tx.wait(),
  );
  if (registerReceipt.status == 0) throw new Error("Auction master registration failed");
